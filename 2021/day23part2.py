...
def next_states(cost,state):
    ...
    for a in am:
        ...
        if y>0: #if we stand in column
            ...
            if x!=goal or not all([typ==tiles[(q,x)] if len(range(y+1,5))>0 else True for q in range(y+1,5)]): # and we are in the wrong or not nicely stacked correct column
                ...
                if y==1 or all(["."==tiles[(c,x)] for c in range(1,y)]):
                    ...
                    if goal!=x:
                        if passebal(x,goal,tiles)==1: 
                            ...
                            goal_room = [tiles[(i,goal)] for i in range(1,5)]

                            if goal_room==[".",typ,typ,typ]:
                                good_moves.append((cc_cost+1,(a,1,goal)))
                            elif goal_room==[".",".",typ,typ]:
                                good_moves.append((cc_cost+2,(a,2,goal)))
                            elif goal_room==[".",".",".",typ]:
                                good_moves.append((cc_cost+3,(a,3,goal)))
                            elif goal_room==[".",".",".","."]:
                                good_moves.append((cc_cost+4,(a,4,goal)))
